...
$Xaml = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" Title="InstallSon II" Height="400" Width="525" HorizontalAlignment="Left" VerticalAlignment="Top" Margin="0,0,0,0">
<Grid>
...
if (Test-Path "\\$Poste.text\c$\temp\DUP_$Model\DELLMUP.exe")
{
 ...
 if (Test-Path "\\cw01pnmtst00\IP\Domaines clients\CR NMP\Drivers\DUP_$Model")
    ...
    ROBOCOPY "$SourceF" "\\$ComputerName\c$\temp\Drivers" /E | %{$data = $_.Split([char]9); if("$($data[4])" -ne "") { $file = "$($data[4])"} ;Write-Progress "Pourcentage copié: $($data[0])" -Activity "Copie des drivers" -CurrentOperation "$($file)"  -ErrorAction SilentlyContinue;}
...
{
    Invoke-Item "\\cw01pnmtst00\IP\Domaines clients\CR NMP\Drivers\version.jpeg"
}
...
{
$ComputerName = $Poste.Text
Start-Process printmanagement.msc /$ComputerName
}
...
{   $ComputerName = $Poste.Text
    ...
    $UserName = $Credential.UserName
    $Password = $Credential.GetNetworkCredential().Password
    .\PsExec.exe \\$ComputerName -u $UserName -p $Password -h cmd /c msiexec /i "\\cw01pnmtst00\IP\Domaines clients\CR NMP\Drivers\DisplayLink_Win10RS.msi" /norestart /quiet
    $Retour.Text = "Display Link installé."
    $Retour.Text = $Error
}
 function Versioninfo(){
 invoke-item "\\cw01pnmtst00\IP\Domaines clients\CR NMP\Drivers\Versioninfos.jpeg"
 }
#endregion
