...
$Xaml = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" Title="Drivers Realtek" Height="283.181" Width="549.771" Background="#FF2B1D1D" BorderBrush="#FF191919">
    <Window.Effect>
        <DropShadowEffect/>
    </Window.Effect>
    <Grid Background="#bbbfbc">
        ...
        <Button Content="Installer les pilotes" HorizontalAlignment="Left" Margin="362.52,131.947,0,0" Name="BInstall" VerticalAlignment="Top" Width="138.844" Height="57.053" Background="#FF4BD63D" BorderBrush="Black" FontWeight="Bold" FontSize="14" VerticalContentAlignment="Center" HorizontalContentAlignment="Center" UseLayoutRounding="False"> 
            <Button.Effect>
                <DropShadowEffect/>
            </Button.Effect>
        </Button>
...
function FOk(){
$Poste=$TPoste.Text
$Model=(Get-WmiObject -ComputerName $Poste Win32_ComputerSystem).Model
...
if (!(test-path .\logs)){mkdir -p logs}
...
"Versions avant installation $DriverVers" >> $LogFile
 if (Test-Path "\\cw01pnmtst00\IP\Domaines clients\CR NMP\Drivers\DUP_$Model")
    ...
    $Drivers1 = Get-WmiObject Win32_PnPSignedDriver -ComputerName $Poste | Select-Object -Property DriverVersion, Manufacturer
    $DriverVers1 = $Drivers1 | Where-Object { $_.Manufacturer -like "*Realtek*" } | Out-String
    "Versions après installation $DriverVers1" >> $LogFile
...
$Poste=$TPoste.Text
$Model=(Get-WmiObject -ComputerName $Poste Win32_ComputerSystem).Model
$Mod=$Model.substring(9)
$ListeVersion = Get-Content '\\cw01pnmtst00\IP\Domaines clients\CR NMP\Drivers\versions.ini' | select -skip 1 |  ConvertFrom-StringData
$TarVers=($ListeVersion).$Mod
$Drivers = Get-WmiObject Win32_PnPSignedDriver -ComputerName $Poste | Select-Object -Property DriverVersion, Manufacturer
$DriverVers = $Drivers | Where-Object { $_.Manufacturer -like "*Realtek*" } | Out-String
Write-Host  "Pilote actuellement installé "$DriverVers "version cible" $TarVers }
...
$tarVersion = Get-Content '\\cw01pnmtst00\IP\Domaines clients\CR NMP\Drivers\versions.ini' | select -skip 1 |  ConvertFrom-StringData
